// File purpose: Display a single food entry. Delete-only — re-add via LogItemModal for changes.
// Related: MealCard.jsx renders this.

import { Trash2 } from 'lucide-react';

export function FoodItem({ food, onDelete }) {
  return (
    <div className="flex items-start gap-2 text-xs group">
      <div className="flex-1 min-w-0">
        <div className="flex items-baseline gap-1.5 flex-wrap">
          <span className="font-semibold text-[var(--color-text-primary)]">{food.name}</span>
